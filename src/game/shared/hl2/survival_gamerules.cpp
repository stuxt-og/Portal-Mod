...
#ifdef CLIENT_DLL
#define CPortalSurvival C_HalfLife2Survival
#define CPortalSurvivalProxy C_HalfLife2SurvivalProxy
#endif

ConVar gamerules_survival( "gamerules_survival", "0", FCVAR_REPLICATED );

class CPortalSurvivalProxy : public CGameRulesProxy
{
public:
	DECLARE_CLASS( CPortalSurvivalProxy, CGameRulesProxy );
...
class CPortalSurvival : public CPortal
{
public:
	DECLARE_CLASS( CPortalSurvival, CPortal );
...
//-----------------------------------------------------------------------------
inline CPortalSurvival* HL2SurvivalGameRules()
{
	return static_cast<CPortalSurvival*>(g_pGameRules);
}

REGISTER_GAMERULES_CLASS( CPortalSurvival );

BEGIN_NETWORK_TABLE_NOBASE( CPortalSurvival, DT_HL2SurvivalGameRules )
END_NETWORK_TABLE()


LINK_ENTITY_TO_CLASS( hl2_survival_gamerules, CPortalSurvivalProxy );
IMPLEMENT_NETWORKCLASS_ALIASED( HalfLife2SurvivalProxy, DT_HalfLife2SurvivalProxy )
	...
	{
		CPortalSurvival *pRules = HL2SurvivalGameRules();
		Assert( pRules );
		*pOut = pRules;
	}

	BEGIN_RECV_TABLE( CPortalSurvivalProxy, DT_HalfLife2SurvivalProxy )
	...
	BEGIN_SEND_TABLE( CPortalSurvivalProxy, DT_HalfLife2SurvivalProxy )
...
CPortalSurvival::CPortalSurvival()
{
	m_bActive = false;
}

void CPortalSurvival::Think( void )
{

}

bool CPortalSurvival::IsAllowedToSpawn( CBaseEntity *pEntity )
{
	if ( !m_bActive )
...
void CPortalSurvival::PlayerSpawn( CBasePlayer *pPlayer )
{
...
void CPortalSurvival::ParseSurvivalSettings( KeyValues *pSubKey )
{
	if ( pSubKey == NULL )
		return;
...
void CPortalSurvival::ParseSurvivalAmmo( KeyValues *pSubKey )
{
	if ( pSubKey )
	{
...
void CPortalSurvival::ReadSurvivalScriptFile( void )
{
	char szFullName[512];
...
void CPortalSurvival::CreateStandardEntities( void )
{
