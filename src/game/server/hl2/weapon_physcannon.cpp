...
void CPlayerPickupController::Init( CBasePlayer *pPlayer, CBaseEntity *pObject )
{
	...
	CPortal_Player *pOwner = (CPortal_Player *)ToBasePlayer( pPlayer );
	if ( pOwner )
	{
#ifndef MAPBASE
...
void CPlayerPickupController::Shutdown( bool bThrown )
{
	...
	if ( m_pPlayer )
	{
		CPortal_Player *pOwner = (CPortal_Player *)ToBasePlayer( m_pPlayer );
#ifndef MAPBASE
		if ( pOwner )
		{
			pOwner->EnableSprint( true );
...
void CWeaponPhysCannon::DoEffectIdle( void )
{
	...
	if ( m_bPhyscannonState != IsMegaPhysCannon() )
	{
		...
		if ( m_bPhyscannonState == false )
		{
			...
			CPortal_Player *pPlayer = dynamic_cast<CPortal_Player*>( pOwner );
