...
{	
	...
	C_BasePortalPlayer *pLocalPlayer = (C_BasePortalPlayer *)C_BasePlayer::GetLocalPlayer();
	if ( pLocalPlayer )
	{
		if( !pLocalPlayer->m_PortalLocal.m_bDisplayReticle )
		{
			return false;
		}
	}
...
void CHUDAutoAim::OnThink()
{
	...
	CBaseEntity *pTarget = pLocalPlayer->m_PortalLocal.m_hAutoAimTarget.Get();
	...
	if( pLocalPlayer->m_PortalLocal.m_bZooming || pLocalPlayer->m_PortalLocal.m_bWeaponLowered )
	{
	...
	case AUTOAIM_METHOD_RETICLE:
		{
			if( pLocalPlayer->m_PortalLocal.m_hAutoAimTarget.Get() && pLocalPlayer->m_PortalLocal.m_bStickyAutoAim )
			{
				if( !pLocalPlayer->IsInAVehicle() )
				{
					...
					if( flSpeed >= 100 && fabs(flDot) <= 0.707f )
					{
						...
						Vector vecDir = pLocalPlayer->m_PortalLocal.m_vecAutoAimPoint - pLocalPlayer->EyePosition();
			...
			if( pTarget )
			{
				...
				else
				{
					...
					ScreenTransform( pLocalPlayer->m_PortalLocal.m_vecAutoAimPoint, screen );
				}
	...
	case AUTOAIM_METHOD_DRIFT:
		{
			if( pLocalPlayer->m_PortalLocal.m_hAutoAimTarget.Get() )
			{
				QAngle viewangles;
...
void CHUDAutoAim::Paint()
{
	...
	if( hud_draw_fixed_reticle.GetBool() )
	{
		...
		C_BasePortalPlayer *pLocalPlayer = (C_BasePortalPlayer *)C_BasePlayer::GetLocalPlayer();
		if( pLocalPlayer && pLocalPlayer->m_PortalLocal.m_hAutoAimTarget.Get() )
		{
			r = 250; 
			g = 138;
			b = 4;
