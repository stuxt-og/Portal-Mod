...
IMPLEMENT_CLIENTCLASS_DT(C_BasePortalPlayer, DT_Portal_Player, CPortal_Player)
...
BEGIN_PREDICTION_DATA( C_BasePortalPlayer )
	DEFINE_PRED_TYPEDESCRIPTION( m_PortalLocal, C_PortaPlayerLocalData ),
...
LINK_ENTITY_TO_CLASS( player, C_BasePortalPlayer );
#endif
...
// Constructor
//-----------------------------------------------------------------------------
C_BasePortalPlayer::C_BasePortalPlayer()
{
...
// Input  : updateType - 
//-----------------------------------------------------------------------------
void C_BasePortalPlayer::OnDataChanged( DataUpdateType_t updateType )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void C_BasePortalPlayer::Weapon_DropPrimary( void )
{
	engine->ServerCmd( "DropPrimary" );
}

float C_BasePortalPlayer::GetFOV()
{
...
// Output : float
//-----------------------------------------------------------------------------
float C_BasePortalPlayer::GetZoom( void )
{
...
//			time - 
//-----------------------------------------------------------------------------
void C_BasePortalPlayer::Zoom( float FOVOffset, float time )
{
...
// Output : int
//-----------------------------------------------------------------------------
int C_BasePortalPlayer::DrawModel( int flags )
{
...
//-----------------------------------------------------------------------------
void C_BasePortalPlayer::ExitLadder()
{
...
//-----------------------------------------------------------------------------
bool C_BasePortalPlayer::TestMove( const Vector &pos, float fVertDist, float radius, const Vector &objPos, const Vector &objDir )
{
	trace_t trUp;
	trace_t trOver;
	trace_t trDown;
...
//-----------------------------------------------------------------------------
void C_BasePortalPlayer::PerformClientSideObstacleAvoidance( float flFrameTime, CUserCmd *pCmd )
{
...
void C_BasePortalPlayer::PerformClientSideNPCSpeedModifiers( float flFrameTime, CUserCmd *pCmd )
{
...
//-----------------------------------------------------------------------------
bool C_BasePortalPlayer::CreateMove( float flInputSampleTime, CUserCmd *pCmd )
{
...
// Purpose: Input handling
//-----------------------------------------------------------------------------
void C_BasePortalPlayer::BuildTransformations( CStudioHdr *hdr, Vector *pos, Quaternion q[], const matrix3x4_t& cameraTransform, int boneMask, CBoneBitList &boneComputed )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
const QAngle& C_BasePortalPlayer::GetRenderAngles( void )
{
	if ( m_flAnimRenderYaw != FLT_MAX )
	{
