...
#ifdef HL2_EPISODIC
	C_BasePortalPlayer *pPlayer = (C_BasePortalPlayer *)C_BasePlayer::GetLocalPlayer();
	if ( pPlayer )
	{
		float flBatteryPower = ( pPlayer->m_PortalLocal.m_flFlashBattery >= 0.0f ) ? ( pPlayer->m_PortalLocal.m_flFlashBattery ) : pPlayer->m_PortalLocal.m_flSuitPower;
